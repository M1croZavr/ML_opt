...
def brent_method(fun_anl, x_l, x_r, eps=1e-5, max_iter=500, print_info=False, record_info=False, plot=False):
    ...
    # Parameters of parabolic equation
    ...
    if plot:
        ...
        plt.plot(X, Y, 'b-')
    if record_info:
        df = pd.DataFrame(columns=['a', 'b', 'x', 'w', 'v', 'fx'])
    else:
        df = None
    for i in range(max_iter):
        g = e
        e = d
        spi_condition = spi_behaving(v, w, x, fv, fw, fx, a, b, g)
        if spi_condition:
        ...
        if fu <= fx:
            ...
            if u >= x:
                ...
                fw = fun.subs(var, w)
        if print_info:
            print(f'k: {i + 1} | a: {a} | b: {b} | x: {x} | w: {w} | v: {v} | fx: {fx}')
        if record_info:
            df = df.append({'a': a, 'b': b, 'x': x, 'w': w, 'v': v, 'fx': fx}, ignore_index=True)
        if plot and a_ and b_ and c_:
            X = np.linspace(x_l, x_r, 15)
            Y = [a_ * x_i ** 2 + b_ * x_i + c_ for x_i in X]
            plt.plot(X, Y, 'r--')
        if d <= eps:
            print('Найдено значение с заданной точностью')
            break
